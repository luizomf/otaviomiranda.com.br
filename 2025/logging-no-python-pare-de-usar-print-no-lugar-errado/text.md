...
Uma coisa que acontece com frequência, e não só comigo, mas com todo mundo que
usa o `root` logger como central dos handlers, é o seguinte: **libs de terceiros
começam a cuspir log no seu log 🫠**.

Esse comportamento pode até ser desejado em alguns contextos (monitoramento
completo, por exemplo). Mas na maioria das vezes, você quer ver **apenas os logs
da sua aplicação**, sem ruído de dependência externa.
...
<h3 id="rich-logs-mais-bonitos-no-console">
  <a href="#rich-logs-mais-bonitos-no-console">
    `rich`: logs mais bonitos no console
  </a>
</h3>

Vamos usar a biblioteca `rich` pra deixar os logs do console mais agradáveis de
ler, com cor, syntax highlight e outros agrados visuais.

Instale com o gerenciador que preferir:
...
<h3 id="exemplo-pratico-logs-de-terceiros-poluindo-sua-saida">
  <a href="#exemplo-pratico-logs-de-terceiros-poluindo-sua-saida">
    Exemplo prático: logs de terceiros poluindo sua saída
  </a>
</h3>

Vamos configurar o `logging` novamente, do zero:
...
Isso pode ser um **problema grave** dependendo da situação. Imagine que você
adiciona um handler que envia logs por e-mail. Se uma lib como o `rich` começar
a vomitar `DEBUG` o tempo inteiro... adeus caixa de entrada (e olá fatura do
SendGrid 😅).

Mas se você entendeu tudo até aqui, talvez já consiga resolver sozinho. E se não
entendeu, tá tudo certo também. A gente vai resolver isso **passo a passo**.
...
Tem várias coisas que a gente pode fazer, mas duas bem simples e eficientes são:

- Subir o `level` do logger que está causando problema
- Isolar o seu logger do `root`

---

**Como subir o `level` de logs com `getLogger`?**

No próprio log, a **segunda coluna** mostra o nome do logger: `markdown_it`!

Muita gente configura seus loggers assim:
...
Exemplo: se você está num pacote chamado `learn_logging`, e o módulo se chama
`config_logger.py`, o nome do logger será:

- `__main__` se executar direto
- `learn_logging.config_logger` se importar de outro lugar

Mas voltando ao problema: se o spam vem de `markdown_it`, basta fazer:

```python
logging.getLogger("markdown_it").setLevel(logging.INFO)
```

Funcionou? Beleza. Mas se continuar brotando log de todos os locais, talvez seja
melhor **isolar completamente** o seu logger do `root`.
...
Pra isso, você precisa configurar os **handlers no seu próprio logger**. Como
estamos usando `basicConfig`, os handlers estavam indo pro `root`. Agora vamos
fazer de outro modo:
...
Ou seja: antes estávamos usando o `root` só porque é padrão. Mas se ele começar
a te atrapalhar, **larga o root e vai com o seu próprio logger.**

Se quiser comprovar que o `root` ainda existe, mas está sem handler:
...
print(f"{root.hasHandlers() = !r}")
# root.hasHandlers() = False
```

E tá tudo certo com isso. Agora é você quem manda nos logs.
