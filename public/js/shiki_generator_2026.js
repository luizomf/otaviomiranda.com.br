(function () {
  ...
  if (!selectLanguage || !selectTheme || !getCodeBtn || !shikiResult || !code) {
    return;
  }
  ...
  async function getShikiResult() {
    try {
      ...
      console.error(e);
      alert(
        'An error occurred while generating the code. Please check the console for details.',
      );
      shikiResult.innerHTML =
        '<p style="color: red;">Error generating code. See console.</p>';
    } finally {
      // Restore state
