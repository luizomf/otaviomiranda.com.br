    ...
    for (const dir of dirs) {
        ...
        if (mdPath) {
            try {
                ...
                let contentAreaHtml = $('.post-content').html() ||
                    $('main').html() ||
                    $('article').html() ||
                ...
                const imgDiff = Math.abs(legacyData.images.length - newData.images.length);
                ...
                if (isSus) {
                    ...
                    console.log(`-> Imagens: HTML=${legacyData.images.length} vs MD=${newData.images.length}`);
                    console.log(`-> Texto length: HTML=${legacyData.text.length} vs MD=${newData.text.length} (Diff: ${charDiff} chars)`);
                    if (imgDiff > 0) {
                        console.log(`   HTML Imagens:`, legacyData.images.slice(0, 3), legacyData.images.length > 3 ? '...' : '');
                        console.log(`   MD Imagens:`, newData.images.slice(0, 3), newData.images.length > 3 ? '...' : '');
                    }
                } else {
